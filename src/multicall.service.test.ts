...
describe('MultiCallService', () => {
            ...
            when(
                ...
                (
                    ...
                    return {
                        ...
                        // @ts-ignore
                        lastSuccessIndex: callData.params[0].length
                    }
                }
            )
        })
    ...
    describe('callByGasLimit() multicall with errors', () => {
        ...
        beforeEach(() => {
            when(
                ...
                (
                    ...
                    return {
                        results,
                        lastSuccessIndex: getLastSuccessIndex(results)
                    }
                }
            )
        })
        ...
        it('test', async () => {
            ...
            // eslint-disable-next-line @typescript-eslint/ban-ts-comment
            // @ts-ignore
            const ethCallArguments = [0, 1, 2, 3, 4, 5].map(
                // eslint-disable-next-line @typescript-eslint/ban-ts-comment
                // @ts-ignore
                (index) => ethCalls.byCallIndex(index)[1]['params'][0]
            )
